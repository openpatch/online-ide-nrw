...
export class MainEmbedded implements MainBase {
  ...
  initScripts() {
    ...
    if (this.config.withFileList) {
      this.fileExplorer = new EmbeddedFileExplorer(
        this.currentWorkspace.moduleStore,
        this.$filesListDiv,
        this
      );
      this.fileExplorer.setFirstFileActive();
      this.scriptList
        .filter((script) => script.type == "hint")
        .forEach((script) => this.fileExplorer.addHint(script));
    } else {
  ...
  readConfig($div: JQuery<HTMLElement>) {
    ...
    if (this.config.withBottomPanel == null) {
      this.config.withBottomPanel =
        this.config.withConsole ||
        this.config.withPCode ||
        this.config.withFileList ||
        this.config.withErrorList;
    }
  ...
  setModuleActive(module: Module) {
    ...
    if (this.config.withFileList && this.fileExplorer.currentFile != null) {
      this.fileExplorer.currentFile.module.editorState =
        this.getMonacoEditor().saveViewState();
    }
  ...
  readScriptsFromJsonStore(id: string) {
          ...
          this.indexedDB.getScript(scriptId, (script) => {
            ...
            if (countDown == 0) {
              ...
              if (that.fileExplorer == null) {
                let modules =
                  that.currentWorkspace.moduleStore.getModules(false);
  ...
  saveScripts() {
    ...
    if (oneNotSaved) {
      for (let module of modules) {
        ...
        this.indexedDB.writeScript(
          scriptId,
          module.getProgramTextFromMonacoModel()
        );
  ...
  initGUI($div: JQuery<HTMLElement>) {
    ...
    this.$resetButton = jQuery(
      '<div class="joe_resetButton jo_button jo_active" title="Code auf Ausgangszustand zurücksetzen">Code Reset</div>'
    );
    ...
    let $buttonOpen = jQuery(
      '<label type="file" class="img_open-file jo_button jo_active"' +
    ...
    let $buttonSave = jQuery(
      '<div class="img_save-dark jo_button jo_active"' +
        'style="margin-right: 8px;" title="Workspace in Datei speichern"></div>'
    );

    $buttonSave.on("click", () => {
      that.saveWorkspaceToFile();
    });
    ...
    if (this.config.withBottomPanel) {
      ...
      new EmbeddedSlider($bottomDiv, true, true, () => {
    ...
    let $infoButton = jQuery(
      '<div class="jo_button jo_active img_ellipsis-dark" style="margin-left: 16px"></div>'
    );
    ...
    setTimeout(() => {
      ...
      this.loadUserSpritesheet().then(() => {
        ...
        this.interpreter.controlButtons.speedControl.setSpeedInStepsPerSecond(
          this.config.speed
        );
        this.startTimer();
      });
  ...
  showProgramPointerPosition(file: File, position: TextPosition) {
    ...
    if (this.config.withFileList) {
      let fileData = this.fileExplorer.files.find(
        (fileData) => fileData.module.file == file
      );
      if (fileData == null) {
        return;
      }
      ...
      this.programPointerModule =
        this.currentWorkspace.moduleStore.getFirstModule();
    }

    let range = {
      startColumn: position.column,
      startLineNumber: position.line,
      endColumn: position.column + position.length,
      endLineNumber: position.line,
    };

    this.getMonacoEditor().revealRangeInCenterIfOutsideViewport(range);
    this.programPointerDecoration = this.getMonacoEditor().deltaDecorations(
  ...
  hideProgramPointerPosition() {
    if (this.getCurrentlyEditedModule() == this.programPointerModule) {
      this.getMonacoEditor().deltaDecorations(
        this.programPointerDecoration,
        []
      );
    }
  ...
  makeFilesDiv(): JQuery<HTMLElement> {
    let $filesDiv = jQuery(
      '<div class="joe_bottomDivFiles jo_scrollable"></div>'
    );

    let $filesHeader = jQuery(
      '<div class="joe_filesHeader jo_tabheading jo_active"  style="line-height: 24px">Programmdateien</div>'
    );

    this.$filesListDiv = jQuery(
      '<div class="joe_filesList jo_scrollable"></div>'
    );
  ...
  compileIfDirty() {
    ...
    if (
      this.currentWorkspace.moduleStore.isDirty() &&
      ...
      try {
        ...
        let errors = this.bottomDiv?.errorManager?.showErrors(
        ...
        let startable =
          this.interpreter.getStartableModule(
            this.currentWorkspace.moduleStore
          ) != null;
        ...
        if (
          !startable &&
          (this.interpreter.state == InterpreterState.done ||
            this.interpreter.state == InterpreterState.error)
        ) {
  ...
  printProgram() {
    this.bottomDiv.printModuleToBottomDiv(
      this.currentWorkspace,
      this.getCurrentlyEditedModule()
    );
  }
  ...
  makeBottomDiv(
    $bottomDiv: JQuery<HTMLElement>,
    $buttonDiv: JQuery<HTMLElement>
  ) {
    let $tabheadings = jQuery('<div class="jo_tabheadings"></div>');
    $tabheadings.css("position", "relative");
    let $thRightSide = jQuery(
      '<div class="joe_tabheading-right jo_noHeading"></div>'
    );

    $thRightSide.append($buttonDiv);

    if (this.config.withConsole) {
      let $thConsoleClear = jQuery(
        '<div class="img_clear-dark jo_button jo_active jo_console-clear"' +
    ...
    if (this.config.withErrorList) {
      let $thErrors = jQuery(
        '<div class="jo_tabheading jo_active" data-target="jo_errorsTab" style="line-height: 24px">Fehler</div>'
      );
      $tabheadings.append($thErrors);
    }

    if (this.config.withConsole) {
      let $thConsole = jQuery(
        '<div class="jo_tabheading jo_console-tab" data-target="jo_consoleTab" style="line-height: 24px">Console</div>'
      );
    ...
    if (this.config.withPCode) {
      let $thPCode = jQuery(
        '<div class="jo_tabheading" data-target="jo_pcodeTab" style="line-height: 24px">PCode</div>'
      );
    ...
    if (this.config.withErrorList) {
      let $tabError = jQuery(
        '<div class="jo_active jo_scrollable jo_errorsTab"></div>'
      );
  ...
  loadWorkspaceFromText(text: string) {
    ...
    if (ew.modules == null || ew.name == null || ew.settings == null) {
      alert(`<div>Das Format passt nicht.</div>`);
      return;
    }
    ...
    if (this.fileExplorer != null) {
      ...
      ws.moduleStore
        .getModules(false)
        .forEach((module) => this.fileExplorer.addModule(module));
      this.fileExplorer.setFirstFileActive();
    } else {
      this.setModuleActive(this.currentWorkspace.moduleStore.getFirstModule());
    }
  ...
  makeRightDiv(): JQuery<HTMLElement> {
    ...
    if (!this.config.hideEditor) {
      ...
      $tabheadings.css("position", "relative");
      let $thRun = jQuery(
        '<div class="jo_tabheading jo_active" data-target="jo_run" style="line-height: 24px">Ausgabe</div>'
      );
      let $thVariables = jQuery(
        '<div class="jo_tabheading jo_console-tab" data-target="jo_variablesTab" style="line-height: 24px">Variablen</div>'
      );
      $tabheadings.append($thRun, $thVariables);
      this.$rightDivInner.append($tabheadings);
      let $vd = jQuery(
        '<div class="jo_scrollable jo_editorFontSize jo_variablesTab"></div>'
      );
  ...
  async loadUserSpritesheet() {
    if (this.config.spritesheetURL != null) {
      ...
      await spritesheet.initializeSpritesheetForWorkspace(
        this.currentWorkspace,
        this,
